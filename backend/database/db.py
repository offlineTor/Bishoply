...
async def migrate_discord_id_columns(db):
    ...
    for table, column in columns:
        ...
        actual_type = str(type_row["data_type"]).lower() if type_row else "missing"
        migration_log.info("%s.%s type=%s", table, column, actual_type)
        if not type_row:
            raise RuntimeError(f"Required Discord ID column missing: {table}.{column}")
        ...
        migration_log.info("%s.%s type=bigint", table, column)

    await record_version(db, DISCORD_ID_MIGRATION_VERSION)


async def migrate_practice_identity_columns(db):
    """Give PostgreSQL Practice rows server-generated IDs without data loss."""
    if getattr(db, "backend", "sqlite") != "postgres":
        return
    ...
    for table in ("practice_games", "practice_moves", "analysis_revisions"):
        ...
        if not column:
            raise RuntimeError(f"Required Practice ID column missing: {table}.id")
        if str(column["is_identity"]).upper() != "YES" and not column["column_default"]:
            await db.execute(
