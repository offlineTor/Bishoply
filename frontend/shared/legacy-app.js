...
function setupWebLab({ STARTING_FEN, createLabState, importLabFen, resetLab, editFen, boardPieces, movePiece, removePiece, normalizeLabAnalysis }) {
  if (!labFen || !labMode) return;
  labState = createLabState();
  labFen.value = labState.fen;
  labMode.value = labState.mode;
  let selectedPiece = "P";
  const renderAnalysis = (response) => {
    const result = response && response.evaluation !== undefined ? response : null;
    if (!result || !labAnalysis) return;
    labAnalysis.innerHTML = `<div class="lab-result-grid"><div><span>Evaluation</span><strong>${result.mate != null ? `Mate in ${Math.abs(result.mate)}` : result.evaluation ?? "—"}</strong></div><div><span>Best Move</span><strong>${escapeHtml(result.bestMove || "—")}</strong></div><div><span>Candidates</span><strong>${escapeHtml((result.candidates || []).join(" · ") || "—")}</strong></div><div><span>Principal Variation</span><strong>${escapeHtml((result.principalVariation || []).join(" ") || "—")}</strong></div><div><span>Depth</span><strong>${result.depth ?? "—"}</strong></div></div>`;
  };
  const render = () => {
    labFen.value = labState.fen;
    labStatus.textContent = labState.error || `Sandbox mode: ${labState.mode}`;
    if (labBoard) { const pieces = boardPieces(labState.fen); labBoard.innerHTML = Array.from({length:64}, (_,i) => { const sq=`${String.fromCharCode(97+i%8)}${8-Math.floor(i/8)}`; return `<button type="button" draggable="${Boolean(pieces[sq])}" data-lab-square="${sq}">${pieces[sq] || ""}</button>`; }).join(""); let dragSource = null; labBoard.querySelectorAll("[data-lab-square]").forEach(btn => { btn.addEventListener("click", () => { labState = editFen(labState, btn.dataset.labSquare, selectedPiece); render(); }); btn.addEventListener("dragstart", event => { dragSource = btn.dataset.labSquare; event.dataTransfer?.setData("text/plain", dragSource); event.dataTransfer?.setData("text/lab-source", dragSource); }); btn.addEventListener("dragend", () => { dragSource = null; labTrash?.classList.remove("is-dragover"); }); btn.addEventListener("dragover", event => event.preventDefault()); btn.addEventListener("drop", event => { event.preventDefault(); const palettePiece = event.dataTransfer?.getData("text/lab-piece"); if (palettePiece) { labState = editFen(labState, btn.dataset.labSquare, palettePiece); render(); return; } const source = event.dataTransfer?.getData("text/lab-source") || dragSource; if (source && source !== btn.dataset.labSquare) { labState = movePiece(labState, source, btn.dataset.labSquare); render(); } }); }); }
  };
  ...
  labPalette?.querySelectorAll("[data-lab-piece]").forEach(button => { button.draggable = true; button.addEventListener("click", () => { selectedPiece = button.dataset.labPiece; labStatus.textContent = selectedPiece ? "Piece selected. Choose a square." : "Remove mode selected."; }); button.addEventListener("dragstart", event => { event.dataTransfer.setData("text/lab-piece", button.dataset.labPiece); }); });
  labTrash?.addEventListener("dragover", event => { if (event.dataTransfer?.types?.includes("text/lab-source")) { event.preventDefault(); labTrash.classList.add("is-dragover"); } });
  labTrash?.addEventListener("dragleave", () => labTrash.classList.remove("is-dragover"));
  labTrash?.addEventListener("drop", event => { event.preventDefault(); labTrash.classList.remove("is-dragover"); const source = event.dataTransfer?.getData("text/lab-source"); if (source) { labState = removePiece ? removePiece(labState, source) : editFen(labState, source, ""); render(); } });
  ...
  labAnalyze?.addEventListener("click", async () => { labAnalyze.disabled = true; labStatus.textContent = "Analyzing position…"; try { const result = await apiFetch("/api/lab/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fen: labState.fen }) }); renderAnalysis(normalizeLabAnalysis(result)); labStatus.textContent = "Analysis ready."; } catch (error) { labStatus.textContent = error.message || "Analysis unavailable."; } finally { labAnalyze.disabled = false; } });
...
function activateProductRoot() {
  ...
  root.hidden = false;
  const productShell = document.createElement("div");
  productShell.className = isDiscordActivity ? "discord-shell" : "web-shell";
  productShell.dataset.product = isDiscordActivity ? "discord" : "web";
  productShell.appendChild(shell);
  root.replaceChildren(productShell);
  const inactiveRoot = document.querySelector(isDiscordActivity ? "#web-app" : "#discord-app");
  inactiveRoot?.replaceChildren();
...
export async function startLegacyApp({ product = isDiscordRuntime() ? "discord" : "web", lab } = {}) {
  ...
  renderEmptyGame();
  activateProductRoot();
  if (!isDiscordActivity && lab) setupWebLab(lab);
  await setupBishoply();
}
